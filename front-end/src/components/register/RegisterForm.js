...
import { getStandard, fetchQuery, DB_HOST } from '../../utilities/fetch'
...
class RegisterForm extends React.Component {
    ...
    async componentDidMount(){
        this.setState({
            organisations: await fetch(DB_HOST+'/api/organisation', getStandard)
                            .then(res=>res.json()),
            clubs:[],
            teams:[],
        })
    ...
    async getClubs(org){
        if(org) this.setState({
                        clubs: await fetch(DB_HOST+'/api/organisation/'+org, getStandard)
                                        .then(res=>res.json())
                                        .then(res=>res.clubs)
    ...
    async getTeams(club){
        if(club) this.setState({
                    teams: await fetchQuery(DB_HOST+'/api/team', {club})
                                    .then(res=>res.json())
                                    .catch(err=>this.props.showSnack(err))
                })
    }
    ...
    checkEmail = ()=>{
        this.state.email && fetch(DB_HOST+'/api/checkemail/'+this.state.email, getStandard)
            .then(res=>res.json())
            .then(res=>{
                if(res.exists) {
